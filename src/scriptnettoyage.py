...
plt.title("Distribution des durées de transit entre ports (60 jours maximum)")
plt.show()

# Compter combien de fois chaque port apparaît (la fréquentation brute)
frequentation = df.groupby("PLACE ID").size().sort_values(ascending=False)
print("Top 20 des ports les plus fréquentés en 2010 :")
print(frequentation.head(20))

# Sur les transits positifs seulement
print(df_sorted[df_sorted["TRANSIT DAYS"] >= 0]["TRANSIT DAYS"].describe())

# Les ports les plus fréquentés dans les données
top_ports = [2581, 1584, 1740, 5717, 9656]

ports_path = base_dir / "data" / "ports_ECA_zones_XY.xlsx"
ports = pd.read_excel(ports_path)

top_ports = [2581, 1584, 1740, 5717, 9656]
print(ports[ports["Port_ID"].isin(top_ports)])
...
print(matching[matching["ID_Lloyds"].isin(top_ports)][["ID_Lloyds", "Name_Final", "COUNTRY", "X1", "Y1"]])
print(matching[matching["ID_Final"].isin(top_ports)][["ID_Final", "Name_Final", "COUNTRY", "X1", "Y1"]])
...
print(df[["PLACE ID", "Name_Final", "COUNTRY"]].drop_duplicates().head(20))